...
unsigned int options_ps; /* Masque des paramètres */
...
/**
 * Récupère les valeurs contenues dans un fichier stat
 * param *d_name : chaine de caractere contenant un PID
...
/**
 * return : 1 si la chaine passé en parametre est un chiffre, 0 sinon
 */ 
...
/**
 * Parcours le répertoire /proc afin de récupérer et d'afficher, pour chaque 
 * répertoire appartenant à un processus (où le nom est un chiffre) et qui 
 * correspond aux requetes effectuées en paramétres, ses propriétes.
 */
void open_proc()
{
   ...
   /* Tentative d'ouverture du répertoire */
   rep = opendir ("/proc");
   if (rep == NULL)
   {
      fprintf (stderr,"myps: Probleme Žà l'ouverture du dossier proc \n");
      exit (1);
   }
...
/* Fermeture du répertoire */
   closedir (rep);
}


/**
 * retourne le numéro du TTY de ce processus, afin de reconnaitre et afficher 
 * les processus du même TTY de myshell
 * BUG : le tty retourné devrait etre sans mask affin de ne pas reconnaitre comme 
 * équivalent les getty des consoles X. Le mask permettant de reconnaitre un TTY de getty
 * m'est inconnu. Pour le paramettre -t <tty>, je compare comme numéro (int) les TTY
...
/**
 * récupére les parametres et place les masks de selection ou d'affichage selon ceux-ci
 */
